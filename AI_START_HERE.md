...
5. **`tests/test_kernel_self_test.py`** (`python3 -m pytest tests/ -q`) — pytest wrapper around
   the same self-test plus the demo/checkpoint-demo fixtures and the newer `review_mode`/agency
   regression tests. At the time this file was written it reported `13/13 PASS`; re-run it.
...
7. **`CHANGELOG.md`** — version history through v0.4.8; states plainly which later milestones
