...
1. **Agency-taxonomy fields ไม่ได้บังคับทั้ง 12 role จริง — เอกสารแค่ไม่เคยพูดชัด** ตรวจสอบแล้วว่า kernel (`AGENCY_ROLE_EMPTY`) บังคับแค่ 5 field (`affected`, `observers`, `decision_owners`, `intervention_owners`, `accountable_parties`) ส่วนอีก 7 field (`voice_holders`, `veto_or_consent_holders`, `oversight_parties`, `represented_or_absent_parties`, `resource_holders`, `knowledge_holders`, `future_or_indirect_parties`, `power_exposure_voice_gaps`) **ไม่เคยถูกบังคับเลย** — โดเมนที่มีผู้เกี่ยวข้องจริงแค่ 2-3 คนคุยกันตรงๆ (เกษตรกรคนเดียว, โค้ชกับนักกีฬา 1 คน, ensemble ดนตรีขนาดเล็ก) ปล่อยให้ 7 field นี้เป็น `[]` ได้เลยโดยไม่ต้อง manufacture เนื้อหา — รายละเอียดเต็มดูที่ `docs/FIELD_REFERENCE.md`'s agency section (auto-generated, ไม่ต้องแก้ตรงนี้อีกถ้า kernel เปลี่ยน)
...
- ยืนยันด้วยการทดสอบจริง (ไม่ใช่แค่อ่านโค้ด) ว่า agency's 7 field ที่ไม่ใช่ 5 field หลัก ไม่เคยถูก kernel บังคับเลย — เพิ่ม permanent regression test + เอกสารใน `docs/FIELD_REFERENCE.md` (§6.9.4)
